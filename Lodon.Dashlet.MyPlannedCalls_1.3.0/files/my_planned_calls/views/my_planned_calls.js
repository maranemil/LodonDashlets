({
    ...
    onCallLink:function(event) {
        ...
        var id = $(event.target).data("obj-id");
        this.resetRowBackground();
        $(event.target).parent().parent().parent().css('background-color','#f5f5f5');

        var url = app.api.buildURL('Calls', id);
        app.api.call('READ', url, {},
        {
            success:function(data) {
                ...
                var d = new Date(data.date_start);
    ...
    onSaveButton:function(event) {
        ...
        var saveObject = this.getCallObject();
        self = this;
        this.resetFormValues();

        var url = app.api.buildURL('Calls', saveObject.id);
    ...
    getCallObject:function() {
        var id = $('#my-planned-calls-description').data("id");
        var description = $('#my-planned-calls-description').val();
        var status = '';
        ...
        var date = $("#my-planned-calls-dp").val().split("-");
        var time = $("#my-planned-calls-tp").val().split(":");
        var date_start = new Date(date[0], date[1]-1, date[2], time[0], time[1]);
        var duration_hours = $("#my-planned-calls-duration-h").val();
        var duration_minutes = $("#my-planned-calls-duration-m").val();
    ...
    fillTbodyContent:function(table, values) {
        table.empty();
        var rowsTemplate = Handlebars.templates["my_planned_calls.rows"];
    ...
    renderContent:function() {
        ...
        if ($("#my-planned-calls-today-label").length == 1) {
            ...
            var content = this.settings.attributes.label;
            var search = ".dashlet-title:contains('"+content+"')";
    ...
    loadData: function (options) {
        ...
        var url = app.api.buildURL('Calls/my_planned_calls');
        app.api.call('READ', url, {},
        {
            success:function(data) {
                self.today = [];
